...
from pyspark.sql.functions import concat_ws, current_date, lit, when
...
df.withColumn("grade", when((df.salary < 4000), lit("A"))
...
spark.sql("select firstname,salary, case salary when salary < 4000 then 'A' else 'B' END as grade from PER").show()
